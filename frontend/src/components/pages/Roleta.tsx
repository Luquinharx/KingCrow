...
export default function Roleta() {
  ...
  const [slots, setSlots] = useState(['‘ÿÙ', '‘ÿÙ', '‘ÿÙ']);
  ...
  const weeklyTotal = isQualified ? 1 : 0; // Quantos giros semanais (gr+Ìtis) s+˙o poss+°veis
  const extraSpins = profile?.extraSpins || 0; // Giros manuais extras

  // carregar giros j+Ì usados NESTA semana de premia+∫+˙o
  useEffect(() => {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm font-serif tracking-wide uppercase mt-1">
                One spin per week ‘«Û Qualification: 5k+ Loot
              </p>
            </div>
          </div>
        </header>
