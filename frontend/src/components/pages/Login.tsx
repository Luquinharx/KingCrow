...
export default function Login() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (code === 'auth/user-not-found' || code === 'auth/wrong-password' || code === 'auth/invalid-credential') {
        ...
        setError(`Erro ao fazer login (${code || 'sem codigo'}). Tente novamente.`);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black font-serif relative overflow-hidden">
      {/* Background Texture */}
      <div className="absolute inset-0 opacity-20 pointer-events-none bg-[url('https://www.transparenttextures.com/patterns/black-felt.png')]" />

      {/* Red Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-yellow-900/20 blur-[100px] rounded-full pointer-events-none animate-pulse" />
      ...
      <div className="w-full max-w-lg mx-auto bg-gray-950/80 border border-white/10 rounded-sm p-8 shadow-[0_0_50px_rgba(0,0,0,0.8)] backdrop-blur-sm relative z-10 mt-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {isBusy ? 'AUTHENTICATING...' : 'ENTER SYSTEM'}
          </button>
        </form>
